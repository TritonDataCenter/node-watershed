...
_ws_readFrame()
{
	...
	if (this._check_for_http_header) {
		 ...
		 * stream, even though it _should_ have been eaten by the
		 * HTTP parser.  Check for that condition on the first
		 * frame.
		 */
		if (this._data.toString('utf8', 0, 2) === 'HT') {
			/*
			 * We need to look for the end of the HTTP header.
			 */
			this._ws_pullup(null);
			var str = this._data.toString('ascii');
			var idx = str.indexOf('\r\n\r\n');
			if (idx === -1)
				return (false);
			/*
			 * Remove what we believe to be the header:
			 */
			this._data = this._data.slice(idx + 4);
			return (true);
		}
		this._check_for_http_header = false;
	}
