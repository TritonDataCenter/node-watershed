...
{
	if (WRAP_SOCKETS) {
		return ((new Readable()).wrap(socket));
	} else {
		return (socket);
	}
}
...
WatershedConnection(options, socket)
{
	...
	self._socket.on('readable', function() {
		 ...
		 */
		while (!self._end_emitted) {
			if (self._ws_readFrame()) {
				self._stats.receivedFrames++;
			} else {
				break;
			}
		}
	...
	/*
	 * Ensure that we get a 'readable' event if there's already
	 * data to be read:
	 */
	self._socket.read(0);
}
