...
_process_frame()
{
	...
	case lib_common.OPCODE.TEXT:
		return ({
			type: 'text',
			payload: wdh.wdh_payload.toString('utf8')
		});

	case lib_common.OPCODE.BINARY:
		return ({
			type: 'binary',
			payload: wdh.wdh_payload
		});

	case lib_common.OPCODE.PING:
		return ({
			type: 'ping',
			payload: wdh.wdh_payload
		});

	case lib_common.OPCODE.PONG:
		return ({
			type: 'pong',
			payload: wdh.wdh_payload
		});

	case lib_common.OPCODE.CLOSE:
