...
_ingest(buf)
{
	...
	PROBES['read-buffer'].fire(function () {
		return ([
			self.wsd_id,
			self.wsd_remote,
			self.wsd_local,
...
_process_frame()
{
	...
	var probe_fire_func = function () {
		return ([
			self.wsd_id,
			self.wsd_remote,
			self.wsd_local,
			wdh.wdh_payload,
			wdh.wdh_payload.length
		]);
	};
	...
	case OPCODE.TEXT:
		PROBES['recv-text'].fire(probe_fire_func);
		return ({
			type: 'text',
	...
	case OPCODE.BINARY:
		PROBES['recv-binary'].fire(probe_fire_func);
		return ({
			type: 'binary',
	...
	case OPCODE.PING:
		PROBES['recv-ping'].fire(probe_fire_func);
		return ({
			type: 'ping',
			payload: wdh.wdh_payload
		});
	...
	case OPCODE.PONG:
		PROBES['recv-pong'].fire(probe_fire_func);
		return ({
			type: 'pong',
			payload: wdh.wdh_payload
		});
	...
	case OPCODE.CLOSE:
		PROBES['recv-close'].fire(probe_fire_func);
