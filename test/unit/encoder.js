...
run_a_test()
{
	...
	var t = TESTS[TEST_ID];
	var objects = t.output;
	var expect = t.bytes;
	var actual = [];

	if (t.fail) {
		/*
		 * Skip failing tests for now.  The fixtures were basically
		 * copied from the encoder tests, and will require some
		 * refactoring for better sharing.
		 */
		TEST_ID++;
	...
	var complete = function (err) {
		...
		if (!mod_jsprim.deepEqual(actual, expect)) {
			console.log('EXPECT: %s', actual.join(' '));
			console.log('ACTUAL: %s', expect.join(' '));
		}
	...
	var we = new lib_encoder.WatershedEncoder({
		...
		local: '2.2.2.2:2000',
		send_mask: t.masked,
		override_mask: t.mask ? new Buffer(t.mask) : undefined
	});

	we.on('readable', function () {
		var buf;
		while ((buf = we.read()) !== null) {
			mod_assert.ok(Buffer.isBuffer(buf), 'not buffer?');

			for (var i = 0; i < buf.length; i++) {
				actual.push(buf[i]);
			}
			console.log('TEST[%s]: output: %s', t.name,
			    buf.inspect());
		}
	});
	we.on('end', function () {
